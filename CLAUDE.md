...
**WebSocket:** 14 services call `websocket.init()` and expose Socket.IO at `/<service-base>/socket.io/`. The F5 NGINX controller strips the `Upgrade` header by default, causing 400 errors on WebSocket handshakes. Add `nginx.org/websocket-services` to `ingress.annotations` listing all WebSocket-enabled services — see the example in values.yaml. ingress-nginx (community) and ALB handle WebSocket transparently with no extra annotation required.
